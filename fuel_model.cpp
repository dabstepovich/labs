...
FuelPrice FuelModel::ParseLine(const QString &line) {
  ...
  if (first_quote == -1 || last_quote == -1 || first_quote == last_quote) {
    throw ParseException(
        QString("Отсутствуют или неверно расставлены кавычки: \"%1\"").arg(line));
  }

  QString fuel_type = line.mid(first_quote + 1, last_quote - first_quote - 1).trimmed();
  ...
  if (!price_ok) {
    throw ParseException(
        QString("Неверный формат цены \"%1\" в строке: \"%2\"")
            .arg(parts[1], line));
  }
  if (price < 0.0) {
    throw ParseException(
...
int FuelModel::LoadFromFile(const QString &file_path, QStringList *parse_errors) {
  prices_.clear();
...
void FuelModel::AddEntry(const FuelPrice &fp) {
  prices_.append(fp);
}
  ...
  if (index < 0 || index >= prices_.size()) {
    throw std::out_of_range(
        QString("RemoveEntry: индекс %1 вне диапазона [0, %2)")
            .arg(index).arg(prices_.size()).toStdString());
  }
